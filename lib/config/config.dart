...
/// with `npx firebase-tools emulators:start` from the `firebase/` directory,
/// so the feature works out of the box during development without touching
...
/// Whether this build can reach a Firebase project: either the local
/// emulators, or real options generated by `flutterfire configure`. When
/// false the Q&A screen explains that the feature is unavailable instead of
/// failing on every request.
bool get isQaConfigured {
  if (useFirebaseEmulators) return true;
  if (!DefaultFirebaseOptions.isSupportedPlatform) return false;
